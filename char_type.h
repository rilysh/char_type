...
{
    return (islower(c) ? (c & 95) : c);
}

/// @description: Convert to lowercase character (without testing
///               the input character).
/// @returns: [tolower_any -> int]
/// @note: tolower_any is not POSIX standard.
template <typename __CType, typename = __Type_IntOrChar<__CType>>
constexpr inline int tolower_any(__CType c) noexcept
{
    return (c | 32);
}

/// @description: Convert to uppercase character (without testing
///               the input character).
/// @returns: [toupper_any -> int]
/// @note: toupper_any is not POSIX standard.
template <typename __CType, typename = __Type_IntOrChar<__CType>>
constexpr inline int toupper_any(__CType c) noexcept
{
    return (c & 95);
}
